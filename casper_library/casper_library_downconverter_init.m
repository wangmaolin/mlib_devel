function casper_library_downconverter_init()
	...
	add_block('built-in/SubSystem', [blk,'/cosin_vanilla']);
	cosin_vanilla_gen([blk,'/cosin_vanilla']);
	set_param([blk,'/cosin_vanilla'], ...
		'output0', sprintf('cos'), ...
		'output1', sprintf('-sin'), ...
		'phase', sprintf('0'), ...
		'fraction', sprintf('0'), ...
		...
		'bram', sprintf('BRAM'), ...
		'misc', sprintf('off'), ...
		'Position', sprintf('[173 100 228 138]'));
...
function cosin_vanilla_gen(blk)

	cosin_vanilla_mask(blk);
	cosin_vanilla_init(blk);
	set_param(blk, ...
		'MaskInitialization', sprintf('cosin_vanilla_init(gcb, ...\n    ''output0'', output0, ...    \n    ''output1'', output1, ...\n    ''phase'', phase, ...\n    ''fraction'', fraction, ...\n    ''table_bits'', table_bits, ...  \n    ''n_bits'', n_bits, ...      \n    ''bin_pt'', bin_pt, ...     \n    ''bram_latency'', bram_latency, ...\n    ''bram'', bram, ...\n    ''misc'', misc);\n'));

end % cosin_vanilla_gen

function cosin_vanilla_mask(blk)

	set_param(blk, ...
		'Mask', sprintf('on'), ...
		'MaskSelfModifiable', sprintf('on'), ...
		'MaskType', sprintf('cosin'), ...
		'MaskPromptString', sprintf('first output function|second output function|initial phase offset (?*(2*pi))|fraction of cycle period to output (1/2^?)|samples in cycle period fraction to output (2^?)|bit width|binary point|bram latency|BRAM implementation|Miscellaneous port'), ...
		'MaskStyleString', sprintf('popup(sin|cos|-sin|-cos),popup(none|sin|cos|-sin|-cos),edit,edit,edit,edit,edit,edit,popup(distributed RAM|BRAM),checkbox'), ...
		'MaskTabNameString', sprintf('basic,basic,basic,basic,basic,basic,basic,latency,implementation,implementation'), ...
		'MaskCallbackString', sprintf('|||||||||'), ...
		...
		'MaskVariables', sprintf('output0=&1;output1=&2;phase=@3;fraction=@4;table_bits=@5;n_bits=@6;bin_pt=@7;bram_latency=@8;bram=&9;misc=&10;'), ...
		'MaskValueString', sprintf('cos|-sin|0|0|0|18|17|1|BRAM|off'), ...
		'BackgroundColor', sprintf('[0.501961, 1.000000, 0.501961]'), ...
		'MaskDisplay', sprintf('if ~(table_bits == 0),\n        color(''black'');port_label(''output'',1,output0);\n    color(''black'');port_label(''output'',2,output1);\n    color(''black'');disp([''z^{-'',num2str(bram_latency),''}''],''texmode'',''on'');color(''black'');port_label(''input'',1,''theta'');\n    if strcmp(misc, ''on''),\n        color(''black'');port_label(''input'',2,''misci'');\n        color(''black'');port_label(''output'',3,''misco'');\n    end\nend'));

end % cosin_vanilla_mask

function cosin_vanilla_init(blk)

end % cosin_vanilla_init
