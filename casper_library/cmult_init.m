...
% conjugated = Whether or not to conjugate the 'a' input.
  ...
  %multipliers 
  if strcmp(multiplier_implementation, 'embedded multiplier core') && strcmp(async, 'off') ...
    && (mult_latency==3) && (add_latency==1)
      % if we're using embedded multipliers, assume that
      % we want to fold in add/subs, and build the cmult directly
      % with DSP Macros.
      % we could use the xilinx cmult block (maybe we should)
      % but this way still allows fanout control (for what that's worth)
      % TODO: deal with the async case

    use_dsp_macros = 1;

    if strcmp(conjugated, 'off'),
      imim_instruction = '-A*B';
      imre_instruction = 'A*B';
    else
      imim_instruction = 'A*B';
      imre_instruction = '-A*B';
    end
    % NB, the space at the end of 'xbsIndex_r4/DSP48 Macro 2.1 ' is
    % necessary!
    reuse_block(blk, 'rere', 'xbsIndex_r4/DSP48 Macro 2.1 ', ...
      'instruction_list', 'A*B+PCIN', ...
      'pipeline_options', 'By_Tier', ...
      'tier_1', 'off', ...
      'tier_2', 'off', ...
      'tier_3', 'on', ...
      'tier_4', 'on', ...
      ...
      'Position', [300 100 400 250]);

    reuse_block(blk, 'imim', 'xbsIndex_r4/DSP48 Macro 2.1 ', ...
      'instruction_list', imim_instruction, ...
      'pipeline_options', 'By_Tier', ...
      'tier_1', 'off', ...
      'tier_2', 'off', ...
      'tier_3', 'off', ...
      ...
      'has_pcout', 'on', ...
      'output_properties', 'Full_Precision', ...
      'Position', [300 300 400 450]);

    reuse_block(blk, 'reim', 'xbsIndex_r4/DSP48 Macro 2.1 ', ...
      'instruction_list', 'A*B+PCIN', ...
      'pipeline_options', 'By_Tier', ...
      'tier_1', 'off', ...
      'tier_2', 'off', ...
      ...
      'has_pcout', 'off', ...
      'output_properties', 'Full_Precision', ...
      'Position', [300 500 400 650]);

    reuse_block(blk, 'imre', 'xbsIndex_r4/DSP48 Macro 2.1 ', ...
      'instruction_list', imre_instruction, ...
      'pipeline_options', 'By_Tier', ...
      'tier_1', 'off', ...
      'tier_2', 'off', ...
      'tier_3', 'off', ...
    ...
    add_line(blk, 'imim/2', 'rere/3');
    add_line(blk, 'imre/2', 'reim/3');
    ...
    if strcmp(multiplier_implementation, 'behavioral HDL'),
      ...
      use_embedded = 'off';
    else strcmp(multiplier_implementation, 'standard core')
  ...
  if use_dsp_macros
    add_line(blk, 'rere/1', 'convert_re/1');
    add_line(blk, 'reim/1', 'convert_im/1');
  else
