...
function pfb_fir_mux_init(blk, varargin)
% Initialize and configure the Polyphase Filter Bank.
%
% pfb_fir_mux_init(blk, varargin)
%
...
for p=1:pols,
	...
	clog(['adding taps for pol ',num2str(p),' input ',num2str(i)],'pfb_fir_mux_init_debug');
        for t=1:TotalTaps,
            ...
            if t==1,
                blk_name = [in_name,'_first_tap_mux'];
                reuse_block(blk, blk_name, 'casper_library_pfbs/first_tap_mux', ...
            ...
            elseif t==TotalTaps,
		        ...
		        reuse_block(blk, blk_name, 'casper_library_pfbs/tap', ...
		            ...
		            'coeff_frac_width',tostring(CoeffBitWidth-1), 'delay', tostring(2^(PFBSize+mux_inputs-n_inputs)), ...
