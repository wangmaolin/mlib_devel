...
function bus_convert_mask(blk)
	...
	set_param(blk, ...
		...
		'MaskPromptString', sprintf('input bit widths|input binary points |input complex|output bit widths|output binary points|quantization strategy (Truncate=0, Round  (unbiased: +/- Inf)=1, Round (unbiased: Even Values)=2)|overflow strategy (Wrap=0, Saturate=1, Flag as error=2)|latency|Adder Implementation|overflow indication|misc support'), ...
		'MaskStyleString', sprintf('edit,edit,checkbox,edit,edit,edit,edit,edit,popup(Fabric|DSP48|Behavioural|Xilinx Cast Block),checkbox,checkbox'), ...
		'MaskTabNameString', sprintf('input,input,input,output,output,output,output,operation,operation,operation,input'), ...
		'MaskCallbackString', sprintf('||||||||||'), ...
		...
		'MaskVariables', sprintf('n_bits_in=@1;bin_pt_in=@2;cmplx=&3;n_bits_out=@4;bin_pt_out=@5;quantization=@6;overflow=@7;latency=@8;adder_imp=&9;of=&10;misc=&11;'), ...
		'MaskValueString', sprintf('[]|8|off|8|4|1|1|2|Fabric|on|on'), ...
		'BackgroundColor', sprintf('[0.501961, 1.000000, 0.501961]'));
...
function bus_single_port_ram_mask(blk)
	...
	set_param(blk, ...
		...
		'MaskPromptString', sprintf('data word bit widths|data word binary points|initial value vector|limit fanout to ?|memory type|memory optimization|asynchronous |misc support|bram latency|input register latency|addr input register|addr register implementation|din input register|din register implementation|we input register|we register implementation|en input register|en register implementation|force we to constant one'), ...
		'MaskStyleString', sprintf('edit,edit,edit,edit,popup(Distributed memory|Block RAM),popup(Area|Speed),checkbox,checkbox,edit,edit,checkbox,popup(core|behavioral),checkbox,popup(core|behavioral),checkbox,popup(core|behavioral),checkbox,popup(core|behavioral),checkbox'), ...
		'MaskTabNameString', sprintf('basic,basic,basic,basic,basic,basic,basic,basic,latency,latency,implementation,implementation,implementation,implementation,implementation,implementation,implementation,implementation,implementation'), ...
		'MaskCallbackString', sprintf('||||||||||||||||||'), ...
