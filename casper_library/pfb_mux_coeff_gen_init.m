...
%
% pfb_mux_coeff_gen_init(blk, varargin)
%
...
function pfb_mux_coeff_gen_init(blk, varargin)


% Declare any default values for arguments you might like.
defaults = {};
if same_state(blk, 'defaults', defaults, varargin{:}), return, end
check_mask_type(blk, 'pfb_mux_coeff_gen');
munge_block(blk, varargin{:});

PFBSize = get_var('PFBSize', 'defaults', defaults, varargin{:});
mux_inputs = get_var('mux_inputs', 'defaults', defaults, varargin{:});
...
try, 
	...
	disp('pfb_mux_coeff_gen_init:Signal Processing Library absent or not working correctly');
	error('pfb_mux_coeff_gen_init:Signal Processing Library absent or not working correctly');
	return;
end
...
reuse_block(blk, 'coeff', 'built-in/outport', 'Position', [550 343 580 357], 'Port', '3');
...
reuse_block(blk, 'Counter', 'xbsIndex_r4/Counter', ...
    'cnt_type', 'Free Running', 'n_bits', tostring(PFBSize+mux_inputs-n_inputs), 'arith_type', 'Unsigned', ...
    'rst', 'on', 'explicit_period', 'on', 'Position', [65 75 115 125]);
%reuse_block(blk, 'Slice', 'xbsIndex_r4/Slice', ...
%    'nbits', tostring(PFBSize-n_inputs), 'mode', 'Lower Bit Location + Width', 'bit1', '0', 'base1', 'MSB of Input', 'bit0', tostring(mux_inputs), 'base0', 'LSB of Input', 'Position', [67 175 100 200]);
reuse_block(blk, 'Slice', 'xbsIndex_r4/Slice', ...
    'nbits', tostring(PFBSize-n_inputs), 'mode', 'Lower Bit Location + Width', 'bit1', '0', 'base1', 'MSB of Input', 'bit0', tostring(mux_inputs), 'base0', 'LSB of Input', 'Position', [137 175 170 200]);
reuse_block(blk, 'Delay1', 'xbsIndex_r4/Delay', ...
    'latency', tostring(bram_latency+1), 'Position', [290 12 335 58]);
reuse_block(blk, 'Concat', 'xbsIndex_r4/Concat', ...
    'num_inputs', tostring(TotalTaps), 'Position', [410 97 465 643]);
reuse_block(blk, 'Register', 'xbsIndex_r4/Register', ...
    'Position', [485 325 530 375]);
...
for a=1:TotalTaps,
    ...
    reuse_block(blk, blkname, 'xbsIndex_r4/ROM', ...
        'depth', tostring(2^(PFBSize-n_inputs)), 'initVector', vecstr, 'arith_type', 'Signed  (2''s comp)', ...
        'n_bits', tostring(CoeffBitWidth), 'bin_pt', tostring(CoeffBitWidth-1), ...
        'latency', tostring(bram_latency), 'use_rpm','on', 'Position', [250 65*(a-1)+74 300 65*(a-1)+126]);
    add_line(blk, 'Slice/1', [blkname, '/1']);
    reintname = ['Reinterpret', tostring(a)];
    reuse_block(blk, reintname, 'xbsIndex_r4/Reinterpret', 'force_arith_type', 'On', ...
        'force_bin_pt','On',...
        'Position', [320 65*(a-1)+84 360 65*(a-1)+116]);
...
fmtstr = sprintf('PFBSize=%d, n_inputs=%d, taps=%d mux=%d', PFBSize, n_inputs, TotalTaps, mux_inputs);
