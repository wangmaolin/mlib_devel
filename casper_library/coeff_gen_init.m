...
function coeff_gen_init(blk, varargin)
  ...
  if length(ActualCoeffs) == 1,
    ...
    if (inorder == 1) || strcmp(coeff_generation, 'off'),
      ...
      %cosin block
      clog(['Deciding whether to use a vanilla cosin block. pack:', pack, ' , multiple_bits:', num2str(multiple_bits), ' , store:', num2str(store)], 'coeff_gen_init_debug');
      if strcmp(pack, 'on') && (multiple_bits == store)
         clog('Using vanilla cosin', 'coeff_gen_init_debug');
         % not sharing brams and packing, don't bother with address generation logic
         reuse_block(blk, 'cosin', 'casper_library_downconverter/cosin_vanilla', ...
          ...
          'bram', bram, 'misc', cosin_misc, ...
          'Position', [280 23 345 147]);
      else
        clog('Using advanced cosin', 'coeff_gen_init_debug');
